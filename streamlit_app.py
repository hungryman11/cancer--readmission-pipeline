...
st.markdown("Upload a CSV or use the **Generate Synthetic Data** button for a demo.")
...
if uploaded_file is None:
    st.info("No file uploaded yet – click below to generate demo data.")
    if st.button("Generate & Download Synthetic Demo Data"):
        ...
        # make high-risk patients look sicker
        high_risk = df['readmitted_30days'] == 1
        df.loc[high_risk, 'age'] += np.random.normal(8, 5, high_risk.sum()).astype(int)
...
# --------------------------------------------------------------
# 2. Load user-uploaded CSV (robust encoding handling)
# --------------------------------------------------------------
@st.cache_data
def load_data(file):
    try:
        ...
        file.seek(0)
        return pd.read_csv(file, encoding='cp1252')   # most common Windows encoding
...
def engineer_features(data: pd.DataFrame) -> pd.DataFrame:
    ...
    risk_cols = [c for c in df.columns if c.startswith(('is_', 'has_', 'prolonged_', 'frequent_', 'emergency_', 'had_', 'anemia', 'hypo', 'polypharmacy'))]
    if risk_cols:
